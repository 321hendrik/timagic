(function () {
	...
	function getInput (obj, key, prompt) {
		...
		rl.question(prompt + '[' + obj[key] + ']' + ' ', function (answer) {
	...
	function ipa (env, settings, tiapp, params) {
		...
		if (params.options.iosv) {
			cmd.concat(['-I', params.options.iosv]);
		}
			...
			return getAndroidDeviceList(settings['adb_path'])
				...
				.then(function (data) {
					...
					if (!androidDeviceList || !androidDeviceList.length) {
						throw new Error('ADB cannot find any connected devices. Try to reconnect devices and run again.');
					}
					return true;
				})
				.then(function () {
					if (!withBuild) {
						return path.join(settings['apk_output_path'], tiapp.get('name') + '.apk');
					}
					return apk(env, settings, tiapp, params);
				})
				...
				.then(function (outputPath) {
					...
					for (var i = 0; i < androidDeviceList.length; i++) {
						(function (i) {
							var promise = apkToDevice(settings['adb_path'], androidDeviceList[i], outputPath)
								// show install status
								.then(function (data) {
									installedToDevices++;
									console.log('\n' + installedToDevices + '/' + androidDeviceList.length, 'devices');
									return data;
								})
					...
					return Q.all(promises)
						.then(function () {
							if (noneInstalled) {
								throw new Error('App could not be installed on any devices.')
							}
						})
					;
				})
				...
				for (var i = 0; i < androidDeviceList.length; i++) {
					(function (i) {
						var promise = removeApkFromDevice(settings['adb_path'], androidDeviceList[i], tiapp.get('id'))
							...
							.then(function (data) {
								removedFromDevices++;
								console.log('\n' + removedFromDevices + '/' + androidDeviceList.length, 'devices');
								return data;
							})
							// check remove status
							.then(function (data) {
