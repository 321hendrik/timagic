(function () {
	...
	function genPLIST (params) {
		...
		var url = params['url'] + params['suffix'] + '/' + encodeURI(params['title']) + '.ipa';
	...
	function genHTML (params) {
		...
		var android_link = project_url + '/' + encodeURI(params['title']) + '.apk';
		var ipa_link = project_url + '/' + encodeURI(params['title']) + '.ipa';
		...
		var promise = mkdirp(distFolder)
			...
			.then(function () {
				return genHTML({
					...
					android: (deploymentTarget.indexOf('android') > -1),
					ios: (deploymentTarget.indexOf('ios') > -1),
				});
			})
			.then(function () {
	...
	/**
	 * Tests if iosDeploy is installed
	 * @return {Boolean} [description]
	 */
	function hasIosDeploy () {
		var cmd = ['which', 'ios-deploy'];
		return execChild(cmd)
			.then(function (data) {
				return {hasIosDeploy: (data.indexOf('not found') === -1)};
			});
	}

	/**
	 * Get list of connected iOS devices (needs iosDeploy)
	 * @return {Promise}         Q-Promise
	 */
	function getIosDeviceList () {
		var cmd = 'ios-deploy -c -t 1 | grep " (" | cut -f2 -d\\( | cut -f1 -d\\)';

		return execChild(cmd)
			.then(function (data) {
				// handle inch in device description e.g. iPad Pro (12.9")
				if (data.indexOf('.') != -1) {
					cmd = 'ios-deploy -c -t 1 | grep " (" | cut -f3 -d\\( | cut -f1 -d\\)';
					return execChild(cmd)
						.then(function(data) {
	...
	function ipaToDevice (deviceId, ipaPath) {
		// var cmd = ['ios-deploy', '--id', deviceId, '--bundle', ipaPath];

		// return execChild(cmd)
		// 	.then(function (data) {
		// 		if (data.indexOf('[100%] Installed') != -1) {
		// 			return {deviceId: deviceId, success: true};
		// 		} else {
		// 			return {deviceId: deviceId, err: data};
		// 		}
		...
		// ipa-deploy version --> @TODO include code from ipa-deploy here
		var cmd = ['ipa-deploy', ipaPath, '-W'];
		...
		return function install (env, settings, tiapp, params) {
			...
			return getAndroidDeviceList(settings['adb_path'])
				...
				.then(function (data) {
					...
					cli.info('Not installing to iOS devices. (Please install ios-deploy --> "homebrew install -g ios-deploy")');
					return true;
				})
				// should build?
				.then(function () {
					...
					if (!withBuild) {
						var appName = tiapp.get('name').split(' ').join('\\ ');
						outputPaths.apk = path.join(settings.apk_output_path, appName + '.apk');
						outputPaths.ipa = path.join(settings.ipa_output_path, appName + '.ipa');
						return outputPaths;
					}
					...
					if (iosDeviceList.length && iosDeployInstalled) {
						promises.push(ipa(env, settings, tiapp, params));
					}
						...
						.then(function (paths) {
							...
							for (var i = 0; i < paths.length; i++) {
								if (paths[i].indexOf('.ipa') !== -1) {
									outputPaths.ipa = paths[i].split(' ').join('\\ ');
								}
								if (paths[i].indexOf('.apk') !== -1) {
									outputPaths.apk = paths[i].split(' ').join('\\ ');
								}
							}
							return outputPaths;
						})
	...
	function removeIpaFromDevice (deviceId, appId) {
		var cmd = ['ios-deploy', '-1', appId, '-9', '-W'];
		// var cmd = ['ideviceinstaller', '-u', deviceId, '-U', appId];

		return execChild(cmd)
			.then(function (data) {
				if (data.indexOf('[ OK ] Uninstalled package') > -1) {
					return {deviceId: deviceId, success: true};
				} else {
					return {deviceId: deviceId, err: data};
				}
	...
	function remove (env, settings, tiapp, params) {
		...
		return getAndroidDeviceList(settings['adb_path'])
			...
			.then(function (data) {
				...
				cli.info('Not installing to iOS devices. (Please install ios-deploy --> "npm install -g ios-deploy")');
				return true;
			})
			.then(function () {
